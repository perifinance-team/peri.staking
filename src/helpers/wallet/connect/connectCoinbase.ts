...
export const connectCoinbase = async (networkId, networkName) => {
	let walletState: WalletState = {
		walletType: SUPPORTED_WALLETS.COINBASE,
		unlocked: false,
		networkId,
		networkName,
	...
	try {
		...
		if (accounts && accounts.length > 0) {
			...
			walletState.unlockReason = 'CoinbaseNoAccounts';
		}
		// We updateWalletStatus with all the infos
	} catch (e) {
		console.log(e);
		walletState.unlockReason = 'ErrorWhileConnectingToCoinbase';
	}
	return walletState;
