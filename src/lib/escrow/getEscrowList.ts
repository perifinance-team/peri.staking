...
export const getEscrowList = async (
	...
	let entryIDs = await RewardEscrowV2.getAccountVestingEntryIDs(
		address,
		0,
		entry
	);
	...
	await Promise.all(
		entryIDs.map(async (entry: object, idx: number) => {
			await RewardEscrowV2.vestingSchedules(address, entry).then(
				(data: object[]) => {
					...
					const toggle =
						new Date().getTime() < parseInt(data[0]["_hex"], 16) * 1000;
