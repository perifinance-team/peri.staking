...
const Vesting = () => {
  ...
  const getGasEstimate = async () => {
    ...
    try {
      gasLimit = BigInt(
        (
          await contracts.signers.PeriFinanceEscrow.estimateGas.vest()
        ).toString()
      );
    } catch (e) {
      console.log(e);
  ...
  const onVest = async () => {
    ...
    try {
      const transaction = await contracts.signers.PeriFinanceEscrow.vest(
        transactionSettings
      );
      dispatch(
        updateTransaction({
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const vestingTotalCount =
        await contracts.PeriFinanceEscrow.numVestingEntries(address);
      const nextVestingIndex: number = (
        await contracts.PeriFinanceEscrow.getNextVestingIndex(address)
      ).toNumber();
      ...
      for await (let data of dataList) {
        let [date, quantity] =
          await contracts.PeriFinanceEscrow.getVestingScheduleEntry(
            address,
            index
          );
      ...
      <TableContainer>
        <StyledTHeader>
          <Row>
            ...
            <Cell>
              <H4 weight={"b"}>QUANTITY</H4>
            </Cell>
            <Cell>
              <H4 weight={"b"}>VESTABLE DATE</H4>
            </Cell>
          </Row>
        </StyledTHeader>
        <StyledTBody height={20}>
          {vestingData.map((e, index) => (
            <BorderRow key={index}>
              <Cell>
                <H4 weight={"m"}>{index + 1}</H4>
              </Cell>
              <Cell>
                <H4 weight={"m"}>
                  {e.quantity === 0n
                    ? "Already Received"
                    : formatCurrency(e.quantity, 18)}
                </H4>
              </Cell>
              <Cell>
                <H4 weight={"m"}>{e.date === 0n ? "-" : e.date}</H4>
              </Cell>
            </BorderRow>
          ))}
        </StyledTBody>
      ...
      <Content>
        <RowContainer>
          ...
          <Input
            height={40}
            currencyName={"PERI"}
            value={formatCurrency(totalVesting, 18)}
            color={"secondary"}
            disabled={true}
          />
        </RowContainer>
