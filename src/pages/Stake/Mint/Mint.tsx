	...
	const getCRatio = (currencyName, mintAmount, stakeAmount) => {
		...
		try {
			let mintAmountToPERI =
				(BigInt(utils.parseEther(mintAmount).toString()) * BigInt(Math.pow(10, 18).toString())) / exchangeRates["PERI"];
			...
			setCRatio(
				(BigInt(Math.pow(10, 18).toString()) * 100n) /
					((totalDEBT * BigInt(Math.pow(10, 18).toString())) / (balances["PERI"].balance + DAIStakedToPERI + USDCStakedToPERI))
			);
		} catch (e) {
			setCRatio(0n);
		}
