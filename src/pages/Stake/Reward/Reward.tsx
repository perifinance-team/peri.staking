...
const Reward = () => {
    ...
    const onClaim = async (name) => {
        ...
        if(name === 'CLAIM') {
            ...
            const claimAble: boolean = currentCRatio === 0n || (BigInt(Math.pow(10, 18).toString()) * 100n / currentCRatio ) >= 399n;
        
            if(!claimAble) {
                NotificationManager.warning('Your C-Ratio is under 400%, Try to burn your debt first', 'ERROR', 0);
                return false;
            }
            try {
                ...
                dispatch(updateTransaction(
                    {
                        ...
                        message: `Claimed rewards ${
                            formatCurrency(claimData.rewards.staking)
                        }`,
                        type: 'CLAIM'
                    }
                ));
            ...
            dispatch(updateTransaction(
                {
                    hash: transaction.hash,
                    message: `LP rewards ${
                        formatCurrency(balances['LP'].rewardEscrow)
                    }`,
                    type: 'LP rewards'
                }
            ));
    ...
    return (
        <Container>
            {slideIndex === 0 && <Title> <H1>REWARD</H1> </Title>}
            <Swiper
                spaceBetween={10}
                direction={'vertical'}
                slidesPerView={4}
                ...
                {actions.map((action, index) => (
                    <SwiperSlide key={action.name} virtualIndex={index}> 
                        <action.component isActive={index === slideIndex} 
                                          actionName={action.name}
                                          periodAction={() => isConnect ? onCloseFeePeriod(action.name) : connectHelp() }
                                          rewardAction={() => isConnect ? onClaim(action.name) : connectHelp() } data={action.data}
